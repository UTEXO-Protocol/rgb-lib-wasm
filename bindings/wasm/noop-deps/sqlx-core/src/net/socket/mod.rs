...
pub async fn connect_tcp<Ws: WithSocket>(
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return Err(io::Error::new(
            ...
            "TCP connect is not supported on wasm32",
        )
        .into());
    }
    ...
    #[cfg(all(not(target_arch = "wasm32"), feature = "_rt-async-std"))]
    {
        ...
        for socket_addr in (host, port).to_socket_addrs()? {
        ...
        // Return the last error we encountered, or a custom error if the hostname didn't resolve to any addresses.
        match last_err {
