...
/// Stub for wasm32: Async::new not used for I/O (sqlx uses async-io for timers/block_on only).
#[cfg(target_arch = "wasm32")]
impl<T> Async<T> {
    pub fn new(_io: T) -> io::Result<Async<T>> {
        Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "Async::new not supported on wasm32",
        ))
    }
}
