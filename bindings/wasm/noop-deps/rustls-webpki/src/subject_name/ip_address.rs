...
pub(crate) fn verify_ip_address_names(reference: &IpAddr, cert: &Cert<'_>) -> Result<(), Error> {
    // Store octets in outer scope so untrusted::Input can borrow them for the whole function.
    let (v4_octets, v6_octets): ([u8; 4], [u8; 16]) = match reference {
        IpAddr::V4(ip) => (ip.octets(), [0u8; 16]),
        IpAddr::V6(ip) => ([0u8; 4], ip.octets()),
    };
    let ip_address = match reference {
        IpAddr::V4(_) => untrusted::Input::from(&v4_octets),
        IpAddr::V6(_) => untrusted::Input::from(&v6_octets),
    };
