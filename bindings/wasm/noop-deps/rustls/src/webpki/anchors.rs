...
use pki_types::{CertificateDer, IntoOwned, TrustAnchor};
...
impl RootCertStore {
    ...
    pub fn add_parsable_certificates<'a>(
        ...
        for der_cert in der_certs {
            let leaked: &'static [u8] =
                Box::leak(der_cert.into_owned().into_boxed_slice());
            let cert_der = CertificateDer::from(leaked);
            #[cfg_attr(not(feature = "logging"), allow(unused_variables))]
            match anchor_from_trusted_cert(&cert_der) {
                Ok(anchor) => {
                    self.roots.push(anchor.to_static());
                    valid_count += 1;
                }
                Err(err) => {
    ...
    pub fn add(&mut self, der: CertificateDer<'_>) -> Result<(), Error> {
        let leaked: &'static [u8] = Box::leak(der.into_owned().into_boxed_slice());
        let cert_der = CertificateDer::from(leaked);
        self.roots.push(
