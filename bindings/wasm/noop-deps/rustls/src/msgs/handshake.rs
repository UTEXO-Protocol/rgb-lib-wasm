    ...
    pub(crate) fn into_owned(self) -> CertificateChain<'static> {
        CertificateChain(
            self.0
                .into_iter()
                .map(|c| {
                    let v: Vec<u8> = c.into_owned();
                    let leaked: &'static [u8] = Box::leak(v.into_boxed_slice());
                    CertificateDer::from(leaked)
                })
...
impl<'a> CertificateEntry<'a> {
    ...
    pub(crate) fn into_owned(self) -> CertificateEntry<'static> {
        CertificateEntry {
            cert: CertificateDer::from({
                let leaked = Box::leak(self.cert.into_owned().into_boxed_slice());
                leaked as &[u8]
            }),
...
impl Codec<'_> for EchConfigContents {
    fn encode(&self, bytes: &mut Vec<u8>) {
        ...
        let name_bytes: &[u8] = (dns_name.as_ref() as &str).as_bytes();
        PayloadU8::<MaybeEmpty>::encode_slice(name_bytes, bytes);
        self.extensions.encode(bytes);
    }
